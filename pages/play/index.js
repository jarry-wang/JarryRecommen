...
Page({
	...
	onLoad: function(param) {
		console.log("playpage--onLoad:"+param.id);
		this.setData({
			currentId: param.id,
		})
		this.idsMap = wx.getStorageSync('ids') || {};
		this.idsArr = Object.keys(this.idsMap);
		this.data.songs = wx.getStorageSync('songs') || {};
		console.log("playpage--onLoad1:"+this.data.songs);
		console.log("playpage--onLoad2:"+this.data.songs[param.id]);
	},
	onReady: function() {
		console.log("playpage--onReady:"+this.data.currentId)
	...
	reload: function(id) {
		var song = this.data.songs[id] || {};
		console.log("reload:"+song.id);
		this.clearTurner();
	...
	addFav: function(favName) {
		...
		favData.picurl = this.data.songs[id].image._url;
